...
var debug   = require('debug')('express-combo:static'),
    ...
    strategy = require('yui-pathogen-decoder'), // custom combo decoding strategy
    yuicombo = require('yui-combo-middleware'), // custom-combo-related middleware

    // Create a composite middleware out of individual middleware
    decoder = yuicombo.createComposite(
        // decode urls encoded using the pathogen strategy, into module names
        yuicombo.decode(strategy),
        // transform module names into module paths
        yuicombo.path()
    ),

    // if no matching mimeType is found use this
...
mapping configuration only.
    ...
    // serving static files from a protected folder where
    // no other files in the folder will be exposed except 
    // for the file specified in the map.
...
module.exports = {
    ...
    @param {Object} group
        ...
        @param {Object} group.urls
        @param {Object} group.config
    **/
    ...
    **/
    ...
    The `mapping` object is of the following form:
    ...
    @method map
    @public
    @param {String} groupName name of the group to associate this mapping with
    @param {Object} urls the url to absolute file mapping (see description)
    @param {Object} config
    ...
    **/
    ...
    be exposed. This will be appended to `config.root` 
    if specified, otherwise will be used as the actual root.
    @param {Object} config the static handler configuration for
    `express.static`.

        @param {String} config.root absolute root path
    ...
    **/
    ...
    @param {Object} config the static handler configuration
        ...
        file extensions and response header for content-type. e.g.
        { 'html': 'text/plain' }
    ...
    **/
    combine: function (config) {
        ...
        var comboSep  = config.comboSep || '~',
            ...
            // Path used for the custom combo route
            customComboPath = '/combo/cc/p';
        ...
        return function (req, res, next) {
            ...
            var body = [],
                urls = [],
                mtypes = config.mimeTypes || null,
                newestStat,
                pending,
                stream,
                customComboRequest;

            // If the request is a custom-encoded combo url, redirect to self
            // with a path-based combo url.
            if (req.url && req.url.indexOf(customComboPath) === 0) {
                // Create a pseudo request object with the `path` property set
                // to a path relative to the route's root path. This relative
                // path is what the decoder expects.
                // /combo/cc/p/core+3.12.0+yui.js => /core+3.12.0+yui.js
                customComboRequest = {
                    path: req.url.slice(customComboPath.length)
                };

                decoder(customComboRequest, res, function (err) {
                    if (err) {
                        res.statusCode = 400;
                        return res.end();
                    }
                    return res.redirect(
                        comboBase + res.locals.paths.join(comboSep)
                    );
                });
                return;
            }
            ...
            if (dedupe(urls.map(function (url) {
                    var mtype,
                        ext;
                    if (!mtypes) {
                        mtype = my.getContentType(url);
                    } else {
                        ext = libpath.extname(url).toLowerCase();
                        mtype = mtypes[ext] || DEFAULT_MIME_TYPE;
                    }
                    return mtype;
                })).length !== 1) {
