...
module.exports = {
    ...
    /**
    For now this is an "alias" for `this."static"`.

    TODO: using the name `share` as `public` is a reserved keyword.
    ...
    **/
    share: function (rootPath, config) {
        return this['static'](rootPath, config);
    },
    ...
    @return {Function} express static middleware
    **/
    map: function (rootPath, mapping) {
        var key,
            fullpath;
        if (typeof rootPath === 'object' && !mapping) {
            mapping = rootPath;

            // use a default appRoot which is the directory 
            // from which the app was started
            rootPath = process.cwd() + '/';
            console.log('using default app dir as rootPath: ' + rootPath);
        }

        if (!mapping) {
            console.error('Missing resource mapping. Path ' + rootPath +
                          ' was not registered with staticHandler');
            return false;
        }
        for (key in mapping) {
            if (mapping.hasOwnProperty(key)) {
                fullpath = mapping[key];
                // skip resources located outside of appRoot
                if (fullpath && fullpath.indexOf(rootPath) === 0) {
                    mapping[key] = fullpath.slice(rootPath.length);
                } else {
                    console.warn('Skipping resource ' + key +
                                 ' as it exists outside of appRoot');
                }
            }
        }
        return this['static'](rootPath, { urls: mapping });
    },
